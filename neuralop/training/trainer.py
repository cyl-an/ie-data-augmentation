            ...
            for idx, sample in enumerate(train_loader):
                ...
                if self.overrides_loss:
                    if isinstance(out, torch.Tensor):
                        loss += self.callbacks.compute_training_loss(out=out.float(), **sample,
                    ...
                    if self.amp_autocast:
                        with amp.autocast(enabled=True):
                            if isinstance(out, torch.Tensor):
                                loss = training_loss(out.float(), y=sample['y'])
                            elif isinstance(out, dict):
                                loss += training_loss(**out, y=sample['y'])
                    else:
                        if isinstance(out, torch.Tensor):
                            loss = training_loss(out.float(), sample['y'])
                        elif isinstance(out, dict):
                            loss += training_loss(**out, y=sample['y'])
            ...
            for idx, sample in enumerate(data_loader):
                ...
                for loss_name, loss in loss_dict.items():
                    if self.overrides_loss:
                        ...
                        if isinstance(out, torch.Tensor):
                            val_loss = loss(out, y=sample['y']).item()
                        elif isinstance(out, dict):
                            val_loss = loss(out, y=sample['y']).item()
